    ...
    GXMVECTOR color)
{
    ...
    batch->Draw(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST, verts, 4);
}
...
void XM_CALLCONV DX::DrawCapsule2D(PrimitiveBatch<VertexPositionColor>* batch, 
    FXMVECTOR origin, 
    FXMVECTOR majorAxis, 
    FXMVECTOR minorAxis, 
    float height,
    FXMVECTOR color)
{
    ...
    vector<VertexPositionColor> vertsUpside((c_ringSegments >> 1) + 1);
    vector<VertexPositionColor> vertsDownside((c_ringSegments >> 1) + 1);
    ...
    XMVECTOR normalizedMajorAxis = XMVector3Normalize(majorAxis);
    XMVECTOR capsuleHeightOffset = normalizedMajorAxis * height * 0.5f;

    for (size_t i = 0; i < (c_ringSegments >> 1); i++)
    {
        XMVECTOR pos = XMVectorMultiplyAdd(majorAxis, incrementalSin, origin + capsuleHeightOffset);
        pos = XMVectorMultiplyAdd(minorAxis, incrementalCos, pos);
        XMStoreFloat3(&vertsUpside[i].position, pos);
        XMStoreFloat4(&vertsUpside[i].color, color);
        // Standard formula to rotate a vector.
        XMVECTOR newCos = incrementalCos * cosDelta - incrementalSin * sinDelta;
        XMVECTOR newSin = incrementalCos * sinDelta + incrementalSin * cosDelta;
        incrementalCos = newCos;
    ...
    for (size_t i = 0; i < (c_ringSegments >> 1); i++)
    {
        XMVECTOR pos = XMVectorMultiplyAdd(majorAxis, incrementalSin, origin - capsuleHeightOffset);
        pos = XMVectorMultiplyAdd(minorAxis, incrementalCos, pos);
        XMStoreFloat3(&vertsDownside[i].position, pos);
        XMStoreFloat4(&vertsDownside[i].color, color);
        // Standard formula to rotate a vector.
        XMVECTOR newCos = incrementalCos * cosDelta - incrementalSin * sinDelta;
        XMVECTOR newSin = incrementalCos * sinDelta + incrementalSin * cosDelta;
        incrementalCos = newCos;
    ...
    vertsUpside[c_ringSegments >> 1] = vertsDownside[0];
    XMStoreFloat3(&vertsUpside[c_ringSegments >> 1].position, XMLoadFloat3(&vertsUpside[c_ringSegments >> 1].position) + capsuleHeightOffset * 2.f);
    vertsDownside[c_ringSegments >> 1] = vertsUpside[0];
    XMStoreFloat3(&vertsDownside[c_ringSegments >> 1].position, XMLoadFloat3(&vertsDownside[c_ringSegments >> 1].position) - capsuleHeightOffset * 2.f);

    vector<VertexPositionColor> verts;
    verts.reserve(c_ringSegments + 3);
    verts = vertsUpside;
    verts.insert(verts.end(), vertsDownside.begin(), vertsDownside.end());
    verts.push_back(vertsUpside[0]);

    batch->Draw(D3D_PRIMITIVE_TOPOLOGY_LINESTRIP, verts.data(), c_ringSegments + 3);
}

void XM_CALLCONV DX::DrawCapsule3D(PrimitiveBatch<VertexPositionColor>* batch,
    FXMVECTOR origin,
    FXMVECTOR direction,
    float radius,
    float height,
    FXMVECTOR color)
{
    ...
    XMVECTOR majorAxis = normalizedDirection * radius;

    /* direction 벡터가 x축 방향과 가깝지 않으면 x축을 외적 연산을 위한 helper 벡터로 사용, 그렇지 않으면 y축을 사용 */
    XMVECTOR helper = (fabs(XMVectorGetX(normalizedDirection)) < 0.9f)
        ? XMVectorSet(1.f, 0.f, 0.f, 0.f)
        : XMVectorSet(0.f, 1.f, 0.f, 0.f);

    XMVECTOR perpendicularA = XMVector3Normalize(XMVector3Cross(normalizedDirection, helper)) * radius;
    XMVECTOR perpendicularB = XMVector3Normalize(XMVector3Cross(normalizedDirection, perpendicularA)) * radius;


    DrawCapsule2D(batch, origin, majorAxis, perpendicularA, height, color);
    DrawCapsule2D(batch, origin, majorAxis, perpendicularB, height, color);

    XMVECTOR offset = normalizedDirection * height * 0.5f;
    DrawRing(batch, origin + offset, perpendicularA, perpendicularB, color);
    DrawRing(batch, origin - offset, perpendicularA, perpendicularB, color);
}
