...
CEffect_Player_AttSwipe::CEffect_Player_AttSwipe(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Player_AttSwipe::CEffect_Player_AttSwipe(const CEffect_Player_AttSwipe& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Player_AttSwipe::Initialize_Prototype()
{
...
HRESULT CEffect_Player_AttSwipe::Initialize(void* pArg)
{
...
void CEffect_Player_AttSwipe::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Player_AttSwipe::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
void CEffect_Player_AttSwipe::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Player_AttSwipe::Render()
{
...
HRESULT CEffect_Player_AttSwipe::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Player_AttSwipe::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Player_AttSwipe::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
CEffect_Player_AttSwipe* CEffect_Player_AttSwipe::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Player_AttSwipe* pInstance = new CEffect_Player_AttSwipe(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Player_AttSwipe");
...
CCloneable* CEffect_Player_AttSwipe::Clone(void* pArg)
{
	CEffect_Player_AttSwipe* pInstance = new CEffect_Player_AttSwipe(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Player_AttSwipe");
		Safe_Release(pInstance);
	}
...
void CEffect_Player_AttSwipe::Free()
{
	super::Free();
