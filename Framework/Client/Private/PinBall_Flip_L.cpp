...
CPinBall_Flip_L::CPinBall_Flip_L(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
    : super{ _pDevice, _pContext }
{
}

CPinBall_Flip_L::CPinBall_Flip_L(const CPinBall_Flip_L& _Prototype)
    : super{ _Prototype }
{
}

HRESULT CPinBall_Flip_L::Initialize_Prototype()
{
...
HRESULT CPinBall_Flip_L::Initialize(void* _pArg)
{
...
void CPinBall_Flip_L::Priority_Update(_float fTimeDelta)
{
}

void CPinBall_Flip_L::Update(_float fTimeDelta)
{
    ...
    Sour = XMVectorSetX(Sour, i - 2.5f);

    m_pColliderCom->Update(temp);
}

void CPinBall_Flip_L::Late_Update(_float _fTimeDelta)
{
...
HRESULT CPinBall_Flip_L::Render()
{
...
void CPinBall_Flip_L::On_CollisionEnter(CGameObject* pGameObject, _uint iGroup)
{
    switch (iGroup)
    {
    ...
    m_pGameInstance->Play_Sound(L"Pinbarge_Flipper2.wav", (_uint)SOUND_CHANNEL::MAPOBJECT, 1.f);

}

void CPinBall_Flip_L::On_Collision(CGameObject* pGameObject, _uint iGroup)
{
    m_bCollision = true;
}

void CPinBall_Flip_L::On_CollisionExit(CGameObject* pGameObject, _uint iGroup)
{
}

HRESULT CPinBall_Flip_L::Ready_Components()
{
    /* Com_Shader */
    ...
    /* Com_Model */
    if (FAILED(__super::Add_Component(LEVEL_PINBALL, TEXT("Prototype_Component_Model_FlipL"),
    ...
    /* Com_Collider */
    {
        ...
        Desc.vCenter = { 0.f, Desc.vExtents.y * 1.5f, 0.f };
...
_vector CPinBall_Flip_L::Get_OffsetPosition()
{
    _matrix temp = m_pTransformCom->Get_WorldMatrix();
    _vector& Sour = temp.r[(_uint)CTransform::STATE::POSITION];
    _float i = XMVectorGetX(Sour);
...
CPinBall_Flip_L* CPinBall_Flip_L::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CPinBall_Flip_L* pInstance = new CPinBall_Flip_L(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed To Create : CPinBall_Flip_L");
...
CCloneable* CPinBall_Flip_L::Clone(void* pArg)
{
    CPinBall_Flip_L* pInstance = new CPinBall_Flip_L(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed To Clone : CPinBall_Flip_L");
        Safe_Release(pInstance);
    }
