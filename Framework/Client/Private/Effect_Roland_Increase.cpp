...
CEffect_Roland_Increase::CEffect_Roland_Increase(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Roland_Increase::CEffect_Roland_Increase(const CEffect_Roland_Increase& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Roland_Increase::Initialize_Prototype()
{
...
HRESULT CEffect_Roland_Increase::Initialize(void* pArg)
{
...
void CEffect_Roland_Increase::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Roland_Increase::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
void CEffect_Roland_Increase::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Roland_Increase::Render()
{
...
HRESULT CEffect_Roland_Increase::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Roland_Increase::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Roland_Increase::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
_bool CEffect_Roland_Increase::Call_Trigger_Event(_float fTimeDelta)
{

	return false;
}

CEffect_Roland_Increase* CEffect_Roland_Increase::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Roland_Increase* pInstance = new CEffect_Roland_Increase(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Roland_Increase");
...
CCloneable* CEffect_Roland_Increase::Clone(void* pArg)
{
	CEffect_Roland_Increase* pInstance = new CEffect_Roland_Increase(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Roland_Increase");
		Safe_Release(pInstance);
	}
...
void CEffect_Roland_Increase::Free()
{
	super::Free();
