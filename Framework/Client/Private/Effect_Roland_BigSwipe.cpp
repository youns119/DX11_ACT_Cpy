...
CEffect_Roland_BigSwipe::CEffect_Roland_BigSwipe(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Roland_BigSwipe::CEffect_Roland_BigSwipe(const CEffect_Roland_BigSwipe& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Roland_BigSwipe::Initialize_Prototype()
{
...
HRESULT CEffect_Roland_BigSwipe::Initialize(void* pArg)
{
	super::DESC* pDesc = static_cast<super::DESC*>(pArg);
	if (nullptr == pDesc)
		return E_FAIL;

	pDesc->EffectDesc.GameObjectDesc.fRotationPerSec = -16.f;
	...
	// m_vOriginScale = _float3(1.65f, 1.f, 1.65f);
...
void CEffect_Roland_BigSwipe::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Roland_BigSwipe::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
void CEffect_Roland_BigSwipe::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Roland_BigSwipe::Render()
{
...
HRESULT CEffect_Roland_BigSwipe::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Roland_BigSwipe::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Roland_BigSwipe::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
CEffect_Roland_BigSwipe* CEffect_Roland_BigSwipe::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Roland_BigSwipe* pInstance = new CEffect_Roland_BigSwipe(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Roland_BigSwipe");
...
CCloneable* CEffect_Roland_BigSwipe::Clone(void* pArg)
{
	CEffect_Roland_BigSwipe* pInstance = new CEffect_Roland_BigSwipe(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Roland_BigSwipe");
		Safe_Release(pInstance);
	}
...
void CEffect_Roland_BigSwipe::Free()
{
	super::Free();
