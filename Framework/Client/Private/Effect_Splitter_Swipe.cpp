...
CEffect_Splitter_Swipe::CEffect_Splitter_Swipe(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Splitter_Swipe::CEffect_Splitter_Swipe(const CEffect_Splitter_Swipe& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Splitter_Swipe::Initialize_Prototype()
{
...
HRESULT CEffect_Splitter_Swipe::Initialize(void* pArg)
{
	super::DESC* pDesc = static_cast<super::DESC*>(pArg);
	if (nullptr == pDesc)
		return E_FAIL;

	pDesc->EffectDesc.GameObjectDesc.fRotationPerSec = -1.5f;
...
void CEffect_Splitter_Swipe::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Splitter_Swipe::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
void CEffect_Splitter_Swipe::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Splitter_Swipe::Render()
{
...
HRESULT CEffect_Splitter_Swipe::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Splitter_Swipe::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Splitter_Swipe::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
CEffect_Splitter_Swipe* CEffect_Splitter_Swipe::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Splitter_Swipe* pInstance = new CEffect_Splitter_Swipe(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Splitter_Swipe");
...
CCloneable* CEffect_Splitter_Swipe::Clone(void* pArg)
{
	CEffect_Splitter_Swipe* pInstance = new CEffect_Splitter_Swipe(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Splitter_Swipe");
		Safe_Release(pInstance);
	}
...
void CEffect_Splitter_Swipe::Free()
{
	super::Free();
