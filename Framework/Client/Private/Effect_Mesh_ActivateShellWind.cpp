...
CEffect_Mesh_ActivateShellWind::CEffect_Mesh_ActivateShellWind(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Mesh_ActivateShellWind::CEffect_Mesh_ActivateShellWind(const CEffect_Mesh_ActivateShellWind& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Mesh_ActivateShellWind::Initialize_Prototype()
{
...
HRESULT CEffect_Mesh_ActivateShellWind::Initialize(void* pArg)
{
	super::DESC* pDesc = static_cast<super::DESC*>(pArg);
	if (nullptr == pDesc)
		return E_FAIL;

	m_fActivatedTime = 2.5f;
	m_fActivatedTimeAcc = 0.f;
	m_fOriginRotationSpeed = -0.25f;

	pDesc->EffectDesc.GameObjectDesc.fSpeedPerSec = 1.f;
	pDesc->EffectDesc.GameObjectDesc.fRotationPerSec = m_fOriginRotationSpeed;
	...
	m_vOriginScale = pDesc->vScale;
	m_pTransformCom->Set_Scale(m_vOriginScale);

	return S_OK;
}

void CEffect_Mesh_ActivateShellWind::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Mesh_ActivateShellWind::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
	...
	if (m_fLifeTimeAcc >= m_fLifeTime)
	{
		...
		else
		{
			...
			m_pTransformCom->Set_Scale(m_vOriginScale);
			m_pTransformCom->Set_RotationSpeed(m_fOriginRotationSpeed);
		}
	}
...
void CEffect_Mesh_ActivateShellWind::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Mesh_ActivateShellWind::Render()
{
...
HRESULT CEffect_Mesh_ActivateShellWind::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Mesh_ActivateShellWind::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Mesh_ActivateShellWind::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
_bool CEffect_Mesh_ActivateShellWind::Call_Trigger_Event(_float fTimeDelta)
{
	if (true == m_bIsTriggerActivated)
		return true;

	m_fActivatedTimeAcc += fTimeDelta;
	m_pTransformCom->Set_RotationSpeed(m_pTransformCom->Get_RotationSpeed() - fTimeDelta * 5.f);

	_vector vScale = XMLoadFloat3(&m_vOriginScale) * (1.f + m_fActivatedTimeAcc * 1.25f / m_fActivatedTime);
	_float3 vScaledScale{};
	XMStoreFloat3(&vScaledScale, vScale);
	m_pTransformCom->Set_Scale(vScaledScale);
	if (m_fActivatedTimeAcc >= m_fActivatedTime)
	{
		m_fLifeTimeAcc = m_vDissolveStart.y;
...
CEffect_Mesh_ActivateShellWind* CEffect_Mesh_ActivateShellWind::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Mesh_ActivateShellWind* pInstance = new CEffect_Mesh_ActivateShellWind(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Mesh_ActivateShellWind");
...
CCloneable* CEffect_Mesh_ActivateShellWind::Clone(void* pArg)
{
	CEffect_Mesh_ActivateShellWind* pInstance = new CEffect_Mesh_ActivateShellWind(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Mesh_ActivateShellWind");
		Safe_Release(pInstance);
	}
...
void CEffect_Mesh_ActivateShellWind::Free()
{
	super::Free();
