...
CNormie_State_Death::CNormie_State_Death()
{
}

HRESULT CNormie_State_Death::Initialize(void* pObj)
{
#ifdef _DEBUG
...
void CNormie_State_Death::Enter(void* pObj)
{	
	CEnemy_Normie* pNormie = static_cast<CEnemy_Normie*>(pObj);
	...
	m_pModel->SetUp_Animation((_uint)NORMIE_ANIM_INDEX::FLIPDIE, false);

	//CGameInstance::GetInstance()->Play_Sound(L"Sardine Aggro 1.wav", (_uint)SOUND_CHANNEL::ENEMY_VOICE1, 0.5f);
	__super::Enter(pObj);
}

void CNormie_State_Death::Update(_float fTimeDelta, void* pObj)
{
	__super::Update(fTimeDelta, pObj);
}

void CNormie_State_Death::Exit(void* pObj)
{
	__super::Exit(pObj);
}

CNormie_State_Death* CNormie_State_Death::Create(void* pArg)
{
	CNormie_State_Death* pInstance = new CNormie_State_Death();
