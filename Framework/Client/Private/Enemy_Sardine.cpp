...
CEnemy_Sardine::CEnemy_Sardine(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEnemy_Sardine::CEnemy_Sardine(const CEnemy_Sardine& Prototype)
	: super(Prototype)
{
}

HRESULT CEnemy_Sardine::Initialize_Prototype()
{
...
HRESULT CEnemy_Sardine::Initialize(void* pArg)
{
	CEnemy_Sardine::DESC* pDesc = static_cast<DESC*>(pArg);
...
void CEnemy_Sardine::Priority_Update(_float fTimeDelta)
{		
...
void CEnemy_Sardine::Update(_float fTimeDelta)
{				
...
void CEnemy_Sardine::Late_Update(_float fTimeDelta)
{
...
HRESULT CEnemy_Sardine::Render_Shadow()
{
...
void CEnemy_Sardine::On_CollisionEnter(CGameObject* pOther, _uint iGroup)
{	
}

void CEnemy_Sardine::On_Collision(CGameObject* pOther, _uint iGroup)
{
}

void CEnemy_Sardine::On_CollisionExit(CGameObject* pOther, _uint iGroup)
{
}

HRESULT CEnemy_Sardine::Ready_Components()
{
	/* Com_NoiseTexture */
	...
	if (FAILED(Add_Component(LEVEL_STATIC, L"Prototype_Component_Model_Enemy_Sardine",
...
HRESULT CEnemy_Sardine::Ready_Physics()
{
	if (m_pPhysicsControllerCom == nullptr)
	{
...
HRESULT CEnemy_Sardine::Ready_PartObject()
{
	// Horn
	{
...
void CEnemy_Sardine::Chase(_float fTimeDelta)
{
	if (!m_bIsChase)
		return;
...
void CEnemy_Sardine::Select_Action()
{
	if (!m_bIsAggroed) {
...
void CEnemy_Sardine::Damaged(_float fDamage, DAMAGE eDamage, CGameObject* pDamager)
{
	if (m_bIsDead)
		return;
...
HRESULT CEnemy_Sardine::Ready_Events()
{
...
CEnemy_Sardine* CEnemy_Sardine::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEnemy_Sardine* pInstance = new CEnemy_Sardine(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Create : CEnemy_Sardine");
...
CCloneable* CEnemy_Sardine::Clone(void* pArg)
{
	CEnemy_Sardine* pInstance = new CEnemy_Sardine(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEnemy_Sardine");
		Safe_Release(pInstance);
	}
