...
HRESULT CRotatePipe::Initialize(void* _pArg)
{
    ...
    pDesc->GameObjectDesc.TransformDesc.fRotationPerSec = 40.f;
...
void CRotatePipe::Update(_float _fTimeDelta)
{
    if (m_bRender)
    {
        ...
        _float fRadian = XMConvertToRadians(sinf(m_fAccTimeDelta) * 10.f);
...
HRESULT CRotatePipe::Ready_Components()
{
...

    {
        ...
        if (FAILED(Add_Component(LEVEL_FIELD, L"Prototype_Component_Physics_Mesh_Pipe_T_Rotate",
