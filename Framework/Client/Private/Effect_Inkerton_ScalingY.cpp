...
CEffect_Inkerton_ScalingY::CEffect_Inkerton_ScalingY(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Inkerton_ScalingY::CEffect_Inkerton_ScalingY(const CEffect_Inkerton_ScalingY& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Inkerton_ScalingY::Initialize_Prototype()
{
...
HRESULT CEffect_Inkerton_ScalingY::Initialize(void* pArg)
{
	...
	m_vTargetScale = _float3(m_vOriginScale.x * fTime, m_vOriginScale.y * fTime * 2.f, m_vOriginScale.z * fTime);
	m_pTransformCom->Set_Scale(m_vOriginScale);

	return S_OK;
}

void CEffect_Inkerton_ScalingY::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Inkerton_ScalingY::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
	...
	if (m_fLifeTime * 0.5f >= m_fLifeTimeAcc)
	{
		_float fTime = m_fLifeTime * 0.5f;
		_vector vScale = XMVectorLerp(XMLoadFloat3(&m_vOriginScale), XMLoadFloat3(&m_vTargetScale), fTime - pow((fTime - m_fLifeTimeAcc), 2.f));
		XMStoreFloat3(&vScaledScale, vScale);
	}
	else
	{
		_float fTime = m_fLifeTime * 0.5f;
		_vector vScale = XMVectorLerp(XMLoadFloat3(&m_vTargetScale), XMLoadFloat3(&m_vOriginScale), fTime - pow((fTime - (m_fLifeTime - m_fLifeTimeAcc)), 2.f));
		XMStoreFloat3(&vScaledScale, vScale);
	}
...
void CEffect_Inkerton_ScalingY::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Inkerton_ScalingY::Render()
{
...
HRESULT CEffect_Inkerton_ScalingY::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Inkerton_ScalingY::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Inkerton_ScalingY::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
_bool CEffect_Inkerton_ScalingY::Call_Trigger_Event(_float fTimeDelta)
{

	return false;
}

CEffect_Inkerton_ScalingY* CEffect_Inkerton_ScalingY::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Inkerton_ScalingY* pInstance = new CEffect_Inkerton_ScalingY(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Inkerton_ScalingY");
...
CCloneable* CEffect_Inkerton_ScalingY::Clone(void* pArg)
{
	CEffect_Inkerton_ScalingY* pInstance = new CEffect_Inkerton_ScalingY(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Inkerton_ScalingY");
		Safe_Release(pInstance);
	}
...
void CEffect_Inkerton_ScalingY::Free()
{
	super::Free();
