#include "pch.h"
#include "..\Public\Effect_Sprite_Loop.h"

#include "GameInstance.h"

CEffect_Sprite_Loop::CEffect_Sprite_Loop(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CSpriteEffect{ pDevice, pContext }
{

}

CEffect_Sprite_Loop::CEffect_Sprite_Loop(const CEffect_Sprite_Loop& Prototype)
	: CSpriteEffect{ Prototype }
{
}

HRESULT CEffect_Sprite_Loop::Initialize_Prototype()
{
...
HRESULT CEffect_Sprite_Loop::Initialize(void* pArg)
{
...
void CEffect_Sprite_Loop::Priority_Update(_float fTimeDelta)
{
...
void CEffect_Sprite_Loop::Update(_float fTimeDelta)
{
	m_fActiveDelayAcc -= fTimeDelta;
	if (m_fActiveDelayAcc > 0.f)
		return;
...
void CEffect_Sprite_Loop::Late_Update(_float fTimeDelta)
{
	if (m_fActiveDelayAcc > 0.f)
		return;
...
HRESULT CEffect_Sprite_Loop::Render()
{
...
HRESULT CEffect_Sprite_Loop::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
void CEffect_Sprite_Loop::Clear_Info(_float fTimeDelta)
{
	super::Clear_Info(fTimeDelta);
	m_pVIBufferCom->Reset_Life();
}

HRESULT CEffect_Sprite_Loop::Ready_Components(const _wstring& strTextureTag, LEVELID eLevelID, CVIBuffer_Particle_Point::DESC& Desc)
{
	/* Com_Shader */
...
CEffect_Sprite_Loop* CEffect_Sprite_Loop::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Sprite_Loop* pInstance = new CEffect_Sprite_Loop(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Sprite_Loop");
...
CGameObject* CEffect_Sprite_Loop::Clone(void* pArg)
{
	CEffect_Sprite_Loop* pInstance = new CEffect_Sprite_Loop(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CEffect_Sprite_Loop");
		Safe_Release(pInstance);
	}
