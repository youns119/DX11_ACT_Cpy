...
void CDebug_PostProcess::Debug_Bloom()
{
	...
	if (ImGui::Checkbox("Skip", &RenderOption.isSkipBloom))
	{
...
void CDebug_PostProcess::Debug_SSAO()
{
	...
	if (ImGui::Checkbox("Skip", &RenderOption.isSkipSSAO))
	{
		m_pGameInstance->Set_RenderOption(RenderOption);
	}
...
void CDebug_PostProcess::Debug_RadialBlur()
{
	RADIALBLUR_PARAMS RadialBlurParams = m_pGameInstance->Get_RadialBlurParams();

	ImGui::PushID(m_iImGuiID++);
	ImGui::SeparatorText("RadialBlur");

	if (ImGui::DragFloat2("Center", (_float*)&RadialBlurParams.vCenter, 1.f, 0.f, (_float)max(g_iWindowSizeX, g_iWindowSizeY)))
	{
		m_pGameInstance->Set_RadialBlurParams(RadialBlurParams);
	}
	if (ImGui::DragFloat("Density", &RadialBlurParams.fDensity, 0.005f, 0.f, 5.f))
	{
		m_pGameInstance->Set_RadialBlurParams(RadialBlurParams);
	}
	if (ImGui::DragFloat("Intensity", &RadialBlurParams.fIntensity, 0.005f, 0.f, 5.f))
	{
		m_pGameInstance->Set_RadialBlurParams(RadialBlurParams);
	}
	if (ImGui::DragFloat("Radius", &RadialBlurParams.fRadius, 0.005f, 0.f, 0.5f))
	{
		m_pGameInstance->Set_RadialBlurParams(RadialBlurParams);
	}
...
void CDebug_PostProcess::Debug_ShadowMask()
{
	SHADOWMASK_PARAMS ShadowParams = m_pGameInstance->Get_ShadowMaskParams();
	CRenderer::RENDER_OPTION RenderOption = m_pGameInstance->Get_RenderOption();

	ImGui::PushID(m_iImGuiID++);
	ImGui::SeparatorText("Shadow Mask");
	if (ImGui::Checkbox("HardShadow", &RenderOption.isHardShadow))
	{
		m_pGameInstance->Set_RenderOption(RenderOption);
	}


	if (ImGui::DragFloat("Radius", &ShadowParams.fRadius, 0.005f, 0.f, 10.f))
	{
		m_pGameInstance->Set_ShadowMaskParams(ShadowParams);
	}

	BILATERALBLUR_PARAMS BilateralBlurParams = m_pGameInstance->Get_BilateralBlurParams();
	if (ImGui::DragFloat("Depth sigma", &BilateralBlurParams.fDepthSigma, 0.000005f, 0.00f, 1.f, "% .6f"))
	{
		m_pGameInstance->Set_BilateralBlurParams(BilateralBlurParams);
	}
...
{
	CWindowFileDialog* pDialog = CWindowFileDialog::Create(CWindowFileDialog::TYPE::SAVE);

	CWindowFileDialog::FILE_TYPE eFileType = CWindowFileDialog::FILE_TYPE::ATMOSPHERE;
...
{
	CWindowFileDialog* pDialog = CWindowFileDialog::Create(CWindowFileDialog::TYPE::LOAD);

	CWindowFileDialog::FILE_TYPE eFileType = CWindowFileDialog::FILE_TYPE::ATMOSPHERE;
	pDialog->Set_FileType(eFileType);
	pDialog->Set_DefaultExt(L".atmosphere");
	//pDialog->Set_DefaultExt(CWindowFileDialog::s_FileTypes[static_cast<_uint>(eFileT ype)].pszSpec);
