...
CEffect_Player_AttNormal::CEffect_Player_AttNormal(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Player_AttNormal::CEffect_Player_AttNormal(const CEffect_Player_AttNormal& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Player_AttNormal::Initialize_Prototype()
{
...
HRESULT CEffect_Player_AttNormal::Initialize(void* pArg)
{
	super::DESC * pDesc = static_cast<super::DESC*>(pArg);
	if (nullptr == pDesc)
		return E_FAIL;

	pDesc->EffectDesc.GameObjectDesc.fRotationPerSec = 12.5f;
...
void CEffect_Player_AttNormal::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Player_AttNormal::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
void CEffect_Player_AttNormal::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Player_AttNormal::Render()
{
...
HRESULT CEffect_Player_AttNormal::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Player_AttNormal::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Player_AttNormal::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
void CEffect_Player_AttNormal::Clear_Info(_float fTimeDelta)
{
	m_bIsLoopEnd = true;	
}

CEffect_Player_AttNormal* CEffect_Player_AttNormal::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Player_AttNormal* pInstance = new CEffect_Player_AttNormal(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Player_AttNormal");
...
CCloneable* CEffect_Player_AttNormal::Clone(void* pArg)
{
	CEffect_Player_AttNormal* pInstance = new CEffect_Player_AttNormal(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Player_AttNormal");
		Safe_Release(pInstance);
	}
...
void CEffect_Player_AttNormal::Free()
{
	super::Free();
