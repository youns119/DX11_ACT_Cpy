...
CEffect_Mesh_Increase::CEffect_Mesh_Increase(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Mesh_Increase::CEffect_Mesh_Increase(const CEffect_Mesh_Increase& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Mesh_Increase::Initialize_Prototype()
{
...
HRESULT CEffect_Mesh_Increase::Initialize(void* pArg)
{
...
void CEffect_Mesh_Increase::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Mesh_Increase::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
	...
	{
		if (true == m_bIsLoop && false == m_bIsLoopEnd)
			m_fLifeTimeAcc = 0.f;
		else
		{
			super::Clear_Info(0.f);
			m_isActive = false;
			m_fLifeTimeAcc = 0.f;
...
void CEffect_Mesh_Increase::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Mesh_Increase::Render()
{
...
HRESULT CEffect_Mesh_Increase::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Mesh_Increase::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Mesh_Increase::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
void CEffect_Mesh_Increase::Clear_Info(_float fTimeDelta)
{
	m_bIsLoopEnd = true;
	m_fLifeTimeAcc = m_vDissolveStart.y * m_fLifeTime;
}

CEffect_Mesh_Increase* CEffect_Mesh_Increase::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Mesh_Increase* pInstance = new CEffect_Mesh_Increase(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Mesh_Increase");
...
CCloneable* CEffect_Mesh_Increase::Clone(void* pArg)
{
	CEffect_Mesh_Increase* pInstance = new CEffect_Mesh_Increase(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Mesh_Increase");
		Safe_Release(pInstance);
	}
...
void CEffect_Mesh_Increase::Free()
{
	super::Free();
