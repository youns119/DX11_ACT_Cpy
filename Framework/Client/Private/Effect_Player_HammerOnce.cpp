...
CEffect_Player_HammerOnce::CEffect_Player_HammerOnce(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Player_HammerOnce::CEffect_Player_HammerOnce(const CEffect_Player_HammerOnce& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Player_HammerOnce::Initialize_Prototype()
{
...
HRESULT CEffect_Player_HammerOnce::Initialize(void* pArg)
{
	super::DESC* pDesc = static_cast<super::DESC*>(pArg);
	if (nullptr == pDesc)
		return E_FAIL;

	pDesc->EffectDesc.GameObjectDesc.fRotationPerSec = -22.5f;
...
void CEffect_Player_HammerOnce::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Player_HammerOnce::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
void CEffect_Player_HammerOnce::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Player_HammerOnce::Render()
{
...
HRESULT CEffect_Player_HammerOnce::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Player_HammerOnce::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Player_HammerOnce::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
CEffect_Player_HammerOnce* CEffect_Player_HammerOnce::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Player_HammerOnce* pInstance = new CEffect_Player_HammerOnce(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Player_HammerOnce");
...
CCloneable* CEffect_Player_HammerOnce::Clone(void* pArg)
{
	CEffect_Player_HammerOnce* pInstance = new CEffect_Player_HammerOnce(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Player_HammerOnce");
		Safe_Release(pInstance);
	}
...
void CEffect_Player_HammerOnce::Free()
{
	super::Free();
