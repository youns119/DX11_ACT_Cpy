#include "pch.h"
#include "..\Public\Effect_Sprite_Loop_NonBlend.h"

#include "GameInstance.h"

CEffect_Sprite_Loop_NonBlend::CEffect_Sprite_Loop_NonBlend(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CSpriteEffect{ pDevice, pContext }
{

}

CEffect_Sprite_Loop_NonBlend::CEffect_Sprite_Loop_NonBlend(const CEffect_Sprite_Loop_NonBlend& Prototype)
	: CSpriteEffect{ Prototype }
{
}

HRESULT CEffect_Sprite_Loop_NonBlend::Initialize_Prototype()
{
...
HRESULT CEffect_Sprite_Loop_NonBlend::Initialize(void* pArg)
{
...
void CEffect_Sprite_Loop_NonBlend::Priority_Update(_float fTimeDelta)
{
...
void CEffect_Sprite_Loop_NonBlend::Update(_float fTimeDelta)
{
	m_fActiveDelayAcc -= fTimeDelta;
	if (m_fActiveDelayAcc > 0.f)
		return;
...
void CEffect_Sprite_Loop_NonBlend::Late_Update(_float fTimeDelta)
{
	if (m_fActiveDelayAcc > 0.f)
		return;
...
HRESULT CEffect_Sprite_Loop_NonBlend::Render()
{
	...
	if (super::EFFECT_TYPE::SPRITE == m_iEffectType)
	{
		...
		if (true == m_bBlendEnable)
			...
			m_pShaderCom->Begin((_uint)SHADER_PASS_PARTICLE::SPRITE_UV_NONBLEND);
	}
	else
	{
		if (true == m_bBlendEnable)
...
HRESULT CEffect_Sprite_Loop_NonBlend::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
void CEffect_Sprite_Loop_NonBlend::Clear_Info(_float fTimeDelta)
{
	super::Clear_Info(fTimeDelta);
	m_pVIBufferCom->Reset_Life();
}

HRESULT CEffect_Sprite_Loop_NonBlend::Ready_Components(const _wstring& strTextureTag, LEVELID eLevelID, CVIBuffer_Particle_Point::DESC& Desc)
{
	/* Com_Shader */
...
CEffect_Sprite_Loop_NonBlend* CEffect_Sprite_Loop_NonBlend::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Sprite_Loop_NonBlend* pInstance = new CEffect_Sprite_Loop_NonBlend(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Sprite_Loop_NonBlend");
...
CGameObject* CEffect_Sprite_Loop_NonBlend::Clone(void* pArg)
{
	CEffect_Sprite_Loop_NonBlend* pInstance = new CEffect_Sprite_Loop_NonBlend(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CEffect_Sprite_Loop_NonBlend");
		Safe_Release(pInstance);
	}
...
void CEffect_Sprite_Loop_NonBlend::Free()
{
	super::Free();
