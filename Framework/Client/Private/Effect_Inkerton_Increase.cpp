...
CEffect_Inkerton_Increase::CEffect_Inkerton_Increase(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Inkerton_Increase::CEffect_Inkerton_Increase(const CEffect_Inkerton_Increase& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Inkerton_Increase::Initialize_Prototype()
{
...
HRESULT CEffect_Inkerton_Increase::Initialize(void* pArg)
{
	...
	m_vTargetScale = _float3(m_vOriginScale.x * fTime, m_vOriginScale.y * fTime, m_vOriginScale.z * fTime);
	m_pTransformCom->Set_Scale(m_vOriginScale);

	return S_OK;
}

void CEffect_Inkerton_Increase::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Inkerton_Increase::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
	...
	_vector vScale = XMVectorLerp(XMLoadFloat3(&m_vOriginScale), XMLoadFloat3(&m_vTargetScale), m_fLifeTime - pow((m_fLifeTime - m_fLifeTimeAcc), 2.f));
...
void CEffect_Inkerton_Increase::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Inkerton_Increase::Render()
{
...
HRESULT CEffect_Inkerton_Increase::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Inkerton_Increase::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Inkerton_Increase::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
_bool CEffect_Inkerton_Increase::Call_Trigger_Event(_float fTimeDelta)
{

	return false;
}

CEffect_Inkerton_Increase* CEffect_Inkerton_Increase::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Inkerton_Increase* pInstance = new CEffect_Inkerton_Increase(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Inkerton_Increase");
...
CCloneable* CEffect_Inkerton_Increase::Clone(void* pArg)
{
	CEffect_Inkerton_Increase* pInstance = new CEffect_Inkerton_Increase(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Inkerton_Increase");
		Safe_Release(pInstance);
	}
...
void CEffect_Inkerton_Increase::Free()
{
	super::Free();
