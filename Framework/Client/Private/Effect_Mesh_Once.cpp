...
CEffect_Mesh_Once::CEffect_Mesh_Once(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Mesh_Once::CEffect_Mesh_Once(const CEffect_Mesh_Once& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Mesh_Once::Initialize_Prototype()
{
...
HRESULT CEffect_Mesh_Once::Initialize(void* pArg)
{
...
void CEffect_Mesh_Once::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Mesh_Once::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
void CEffect_Mesh_Once::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Mesh_Once::Render()
{
...
HRESULT CEffect_Mesh_Once::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Mesh_Once::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Mesh_Once::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
void CEffect_Mesh_Once::Clear_Info(_float fTimeDelta)
{
	m_bIsLoopEnd = true;
	m_fLifeTimeAcc = m_vDissolveStart.y * m_fLifeTime;
}

CEffect_Mesh_Once* CEffect_Mesh_Once::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Mesh_Once* pInstance = new CEffect_Mesh_Once(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Mesh_Once");
...
CCloneable* CEffect_Mesh_Once::Clone(void* pArg)
{
	CEffect_Mesh_Once* pInstance = new CEffect_Mesh_Once(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Mesh_Once");
		Safe_Release(pInstance);
	}
