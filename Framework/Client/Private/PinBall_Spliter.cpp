...
CPinBall_Spliter::CPinBall_Spliter(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
    : super{ _pDevice, _pContext }
{
}

CPinBall_Spliter::CPinBall_Spliter(const CPinBall_Spliter& _Prototype)
    : super{ _Prototype }
{
}

HRESULT CPinBall_Spliter::Initialize_Prototype()
{
...
HRESULT CPinBall_Spliter::Initialize(void* _pArg)
{
...
void CPinBall_Spliter::Priority_Update(_float fTimeDelta)
{
}

void CPinBall_Spliter::Update(_float fTimeDelta)
{
    if (m_bCollision)
    {
...
void CPinBall_Spliter::Late_Update(_float _fTimeDelta)
{
...
HRESULT CPinBall_Spliter::Render()
{
...
void CPinBall_Spliter::On_CollisionEnter(CGameObject* pGameObject, _uint iGroup)
{
    switch (iGroup)
    {
...
void CPinBall_Spliter::On_Collision(CGameObject* pGameObject, _uint iGroup)
{
    m_bCollision = true;
    m_fAccTimeDelta = 0.f;
    _wstring strLightTag = L"Light_Spliter" + to_wstring(m_iID);
...
void CPinBall_Spliter::On_CollisionExit(CGameObject* pGameObject, _uint iGroup)
{
}

HRESULT CPinBall_Spliter::Ready_Components()
{
    /* Com_Shader */
    ...
    /* Com_Model */
    if (FAILED(__super::Add_Component(LEVEL_PINBALL, TEXT("Prototype_Component_Model_SpliterR"),
    ...
    /* Com_Collider */
    {
        ...
        Desc.vExtents = {2.3f, 3.5f, 11.f};
        Desc.vCenter = { -1.5f, 0.f, 1.5f };
        Desc.vRotation = { 0.f, 31.958f, 0.f };
    ...
    }
    {
        ...
        if (FAILED(Add_Component(LEVEL_PINBALL, L"Prototype_Component_Physics_Mesh_SpliterR",
            L"Com_Physics", reinterpret_cast<CComponent**>(&m_pPhysicsCom), &Desc)))
            {
#ifdef _DEBUG
                OutputDebugString(_wstring(L"Prototype Not Ready : Prototype_Component_Physics_Mesh_SpliterR\n").c_str());
#endif
                return S_OK;
            }
            //return E_FAIL;
...
_vector CPinBall_Spliter::Get_OffsetPosition()
{
    return m_pTransformCom->Get_State(CTransform::STATE::POSITION);
}

CPinBall_Spliter* CPinBall_Spliter::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
    CPinBall_Spliter* pInstance = new CPinBall_Spliter(pDevice, pContext);

    if (FAILED(pInstance->Initialize_Prototype()))
    {
        MSG_BOX("Failed To Create : CPinBall_Spliter");
...
CCloneable* CPinBall_Spliter::Clone(void* pArg)
{
    CPinBall_Spliter* pInstance = new CPinBall_Spliter(*this);

    if (FAILED(pInstance->Initialize(pArg)))
    {
        MSG_BOX("Failed To Clone : CPinBall_Spliter");
        Safe_Release(pInstance);
    }
