...
CEffect_RadialDistortion::CEffect_RadialDistortion(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_RadialDistortion::CEffect_RadialDistortion(const CEffect_RadialDistortion& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_RadialDistortion::Initialize_Prototype()
{
...
HRESULT CEffect_RadialDistortion::Initialize(void* pArg)
{
	...
	m_vOriginScale = _float3(45.f, 45.f, 45.f);
	m_pTransformCom->Set_Scale(m_vOriginScale);
	return S_OK;
}

void CEffect_RadialDistortion::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_RadialDistortion::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
	...
	_float3 vScaledScale{}, vCurrentScale{};
	vCurrentScale = m_pTransformCom->Get_Scale();
	_vector vScale = XMLoadFloat3(&vCurrentScale);
	vScale *= (1.f + (m_pTransformCom->Get_Speed() * fTimeDelta) * cosf(XMVectorGetX(g_XMPi * 0.5f) * (m_fLifeTimeAcc / m_fLifeTime)));
...
void CEffect_RadialDistortion::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_RadialDistortion::Render()
{
	
	return S_OK;
}

HRESULT CEffect_RadialDistortion::Render_Revealage()
{
	
	return S_OK;
}

HRESULT CEffect_RadialDistortion::Render_Distortion()
{
...
HRESULT CEffect_RadialDistortion::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
CEffect_RadialDistortion* CEffect_RadialDistortion::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_RadialDistortion* pInstance = new CEffect_RadialDistortion(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_RadialDistortion");
...
CCloneable* CEffect_RadialDistortion::Clone(void* pArg)
{
	CEffect_RadialDistortion* pInstance = new CEffect_RadialDistortion(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_RadialDistortion");
		Safe_Release(pInstance);
	}
...
void CEffect_RadialDistortion::Free()
{
	super::Free();
