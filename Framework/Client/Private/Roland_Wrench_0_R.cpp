...
CRoland_Wrench_0_R::CRoland_Wrench_0_R(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CRoland_Wrench_0_R::CRoland_Wrench_0_R(const CRoland_Wrench_0_R& Prototype)
	: super(Prototype)
{
}

HRESULT CRoland_Wrench_0_R::Initialize_Prototype()
{
...
HRESULT CRoland_Wrench_0_R::Initialize(void* pArg)
{
	DESC* pDesc = static_cast<DESC*>(pArg);

	if (pDesc == nullptr)
	{
		MSG_BOX("Failed To Create : CRoland_Wrench_0_R");
		return E_FAIL;
	}
	else {
...
void CRoland_Wrench_0_R::Priority_Update(_float fTimeDelta)
{
	__super::Priority_Update(fTimeDelta);
}

void CRoland_Wrench_0_R::Update(_float fTimeDelta)
{
	...
	m_pBottomColliderCom->Update(XMLoadFloat4x4(&m_CombinedWorldMatrix));

	__super::Update(fTimeDelta);
}

void CRoland_Wrench_0_R::Late_Update(_float fTimeDelta)
{
	m_pGameInstance->Add_RenderGroup(CRenderer::RENDERGROUP::NONBLEND, this);

#ifdef _DEBUG
	m_pGameInstance->Add_DebugRenderComponent(CRenderer::DEBUGRENDERGROUP::NONBLEND, m_pColliderCom);
	m_pGameInstance->Add_DebugRenderComponent(CRenderer::DEBUGRENDERGROUP::NONBLEND, m_pBottomColliderCom);
#endif // _DEBUG	
}

HRESULT CRoland_Wrench_0_R::Render()
{
	if (!m_isActive)
		return S_OK;
...
void CRoland_Wrench_0_R::On_CollisionEnter(CGameObject* pOther, _uint iGroup)
{
	CPlayer* pPlayer = dynamic_cast<CPlayer*>(pOther);
	if (pPlayer != nullptr && m_bIsGolfAttack) {
		pPlayer->Damaged(25.f, IDamaged::DAMAGE::DAMAGE_SUPER, m_pOwner);
		CGameInstance::GetInstance()->Play_Sound(L"Roland GolfImpact.wav", (_uint)SOUND_CHANNEL::ENEMY_ATTACK3, 1.f);
	}
	else {
...
void CRoland_Wrench_0_R::On_Collision(CGameObject* pOther, _uint iGroup)
{
}

void CRoland_Wrench_0_R::On_CollisionExit(CGameObject* pOther, _uint iGroup)
{
}

void CRoland_Wrench_0_R::Parried()
{
	dynamic_cast<IParried*>(m_pOwner)->Parried();
}

HRESULT CRoland_Wrench_0_R::Load_From_File(const _char* szFilePath, CModel* pModel)
{
...
HRESULT CRoland_Wrench_0_R::Ready_Components()
{
	// Com_Model
	if (FAILED(Add_Component(LEVEL_STATIC, L"Prototype_Component_Model_Roland_Wrench_0_R",
	...
	// Com_Collider
	{
		...
		if (FAILED(Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Collider_Sphere"),
			L"Com_BottomCollider", reinterpret_cast<CComponent**>(&m_pBottomColliderCom), &Desc)))
			return E_FAIL;

		m_pBottomColliderCom->Set_Active(false);
	}
...
HRESULT CRoland_Wrench_0_R::Bind_ShaderResources()
{
...
CRoland_Wrench_0_R* CRoland_Wrench_0_R::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CRoland_Wrench_0_R* pInstance = new CRoland_Wrench_0_R(pDevice, pContext);
...
CCloneable* CRoland_Wrench_0_R::Clone(void* pArg)
{
	CRoland_Wrench_0_R* pInstance = new CRoland_Wrench_0_R(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CRoland_Wrench_0_R");
		Safe_Release(pInstance);
	}
