...
CEnemy_Normie::CEnemy_Normie(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEnemy_Normie::CEnemy_Normie(const CEnemy_Normie& Prototype)
	: super(Prototype)
{
}

HRESULT CEnemy_Normie::Initialize_Prototype()
{
...
HRESULT CEnemy_Normie::Initialize(void* pArg)
{
	CEnemy_Normie::DESC* pDesc = static_cast<DESC*>(pArg);
...
void CEnemy_Normie::Priority_Update(_float fTimeDelta)
{		
...
void CEnemy_Normie::Update(_float fTimeDelta)
{				
...
void CEnemy_Normie::Late_Update(_float fTimeDelta)
{
...
HRESULT CEnemy_Normie::Render_Shadow()
{
...
void CEnemy_Normie::On_CollisionEnter(CGameObject* pOther, _uint iGroup)
{	
}

void CEnemy_Normie::On_Collision(CGameObject* pOther, _uint iGroup)
{
}

void CEnemy_Normie::On_CollisionExit(CGameObject* pOther, _uint iGroup)
{
}

HRESULT CEnemy_Normie::Ready_Components()
{
	/* Com_NoiseTexture */
	...
	if (FAILED(Add_Component(LEVEL_STATIC, L"Prototype_Component_Model_Enemy_Normie",
	...
	/* Com_FSM */
	{
		...
		if (FAILED(Add_Component(LEVEL_STATIC, L"Prototype_Component_Normie_StateMachine",
...
HRESULT CEnemy_Normie::Ready_Physics()
{
	if (m_pPhysicsControllerCom == nullptr)
	{
...
HRESULT CEnemy_Normie::Ready_PartObject()
{
	// Horn
	{
		CNormie_LeftClaw::DESC LeftClawDesc{};

		LeftClawDesc.PartObjectDesc.pParentWorldMatrix = &m_pTransformCom->Get_WorldMatrix_Ref();
		LeftClawDesc.pSocketMatrix = &m_pModelCom->Get_BoneCombinedMatrix_By_Name("ClawGrip_L");
		LeftClawDesc.pOwner = this;

		if (FAILED(Add_PartObject(L"Part_LeftClaw", CGameInstance::GetInstance()->Get_CurrentLevelIndex(), L"Prototype_GameObject_Normie_LeftClaw", &LeftClawDesc)))
			return E_FAIL;
	}
...
void CEnemy_Normie::Chase(_float fTimeDelta)
{
	if (!m_bIsChase)
		return;
...
void CEnemy_Normie::Select_Action()
{
	if (!m_bIsAggroed) {
		if (m_PatrolPoints.size() > 0)
			m_pFSMCom->Call_NextState(L"Patrol");
		else
			m_pFSMCom->Select_StatesFromGroup((_uint)CNormie_StateMachine::STATE_UNAGGRO);
		return;
	}
	...
	if (fDis < 6) {
		m_pFSMCom->Select_StatesFromGroup((_uint)CNormie_StateMachine::STATE_CLOSE);
	}
	else {		
		m_pFSMCom->Select_StatesFromGroup((_uint)CNormie_StateMachine::STATE_OUTOFRANGE);
	}
}

void CEnemy_Normie::Parried()
{
	Reset_RecoverDelay();
	m_bIsDamaged = true;	
...
void CEnemy_Normie::Damaged(_float fDamage, DAMAGE eDamage, CGameObject* pDamager)
{
	if (m_bIsDead)
		return;
...
HRESULT CEnemy_Normie::Ready_Events()
{
...
HRESULT CEnemy_Normie::Ready_IK()
{
	...
	_int iGroupIndex = 0;
	{
		Set_Segment("Thigh_L_1", -80.f, 1.8f, iGroupIndex);
		Set_Segment("Knee_L_1", -80.f, 1.8f, iGroupIndex);
	} iGroupIndex++;
	{
		Set_Segment("Thigh_L_2", -95.f, 1.8f, iGroupIndex);
		Set_Segment("Knee_L_2", -95.f, 1.8f, iGroupIndex);
	} iGroupIndex++;
	{
		Set_Segment("Thigh_L_3", -120.f, 1.8f, iGroupIndex);
		Set_Segment("Knee_L_3", -120.f, 1.8f, iGroupIndex);
	} iGroupIndex++;
	{
		Set_Segment("Thigh_R_1", 80.f, 1.8f, iGroupIndex);
		Set_Segment("Knee_R_1", 80.f, 1.8f, iGroupIndex);
	} iGroupIndex++;
	{
		Set_Segment("Thigh_R_2", 95.f, 1.8f, iGroupIndex);
		Set_Segment("Knee_R_2", 95.f, 1.8f, iGroupIndex);
	} iGroupIndex++;
	{
		Set_Segment("Thigh_R_3", 120.f, 1.8f, iGroupIndex);
		Set_Segment("Knee_R_3", 120.f, 1.8f, iGroupIndex);
	}

	return S_OK;
}

void CEnemy_Normie::Set_Segment(const _char* szBoneName, _float fBaseAngle, _float fDefaultDistance, _int _iGroupIndex)
{
...
CEnemy_Normie* CEnemy_Normie::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEnemy_Normie* pInstance = new CEnemy_Normie(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Create : CEnemy_Normie");
...
CCloneable* CEnemy_Normie::Clone(void* pArg)
{
	CEnemy_Normie* pInstance = new CEnemy_Normie(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEnemy_Normie");
		Safe_Release(pInstance);
	}
