...
CEffect_Roland_Slash::CEffect_Roland_Slash(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Roland_Slash::CEffect_Roland_Slash(const CEffect_Roland_Slash& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Roland_Slash::Initialize_Prototype()
{
...
HRESULT CEffect_Roland_Slash::Initialize(void* pArg)
{
...
void CEffect_Roland_Slash::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Roland_Slash::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
void CEffect_Roland_Slash::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Roland_Slash::Render()
{
...
HRESULT CEffect_Roland_Slash::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Roland_Slash::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Roland_Slash::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
CEffect_Roland_Slash* CEffect_Roland_Slash::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Roland_Slash* pInstance = new CEffect_Roland_Slash(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Roland_Slash");
...
CCloneable* CEffect_Roland_Slash::Clone(void* pArg)
{
	CEffect_Roland_Slash* pInstance = new CEffect_Roland_Slash(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Roland_Slash");
		Safe_Release(pInstance);
	}
...
void CEffect_Roland_Slash::Free()
{
	super::Free();
