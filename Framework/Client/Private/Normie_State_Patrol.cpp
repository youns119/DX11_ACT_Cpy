...
CNormie_State_Patrol::CNormie_State_Patrol()
{
}

HRESULT CNormie_State_Patrol::Initialize(void* pObj)
{
#ifdef _DEBUG
	...
	m_pParolPoints = static_cast<CEnemy_Normie*>(pObj)->Get_PatrolPoints();
...
void CNormie_State_Patrol::Enter(void* pObj)
{	
	m_pMovement = static_cast<CMovement*>(static_cast<CGameObject*>(pObj)->Find_Component(TEXT("Com_Movement")));

	m_pModel->SetUp_Animation((_uint)NORMIE_ANIM_INDEX::IDLE, true, 0.3f);

	__super::Enter(pObj);
}

void CNormie_State_Patrol::Update(_float fTimeDelta, void* pObj)
{			
...
void CNormie_State_Patrol::Exit(void* pObj)
{
	__super::Exit(pObj);
}

CNormie_State_Patrol* CNormie_State_Patrol::Create(void* pArg)
{
	CNormie_State_Patrol* pInstance = new CNormie_State_Patrol();
...
void CNormie_State_Patrol::Free()
{
	__super::Free();
}