...
CEffect_MeshInst_Once::CEffect_MeshInst_Once(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_MeshInst_Once::CEffect_MeshInst_Once(const CEffect_MeshInst_Once& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_MeshInst_Once::Initialize_Prototype()
{
...
HRESULT CEffect_MeshInst_Once::Initialize(void* pArg)
{
...
void CEffect_MeshInst_Once::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_MeshInst_Once::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
void CEffect_MeshInst_Once::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_MeshInst_Once::Render()
{
...
HRESULT CEffect_MeshInst_Once::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_MeshInst_Once::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_MeshInst_Once::Ready_Components(void* pArg)
{
	super::DESC* pDesc = static_cast<super::DESC*>(pArg);
...
CEffect_MeshInst_Once* CEffect_MeshInst_Once::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_MeshInst_Once* pInstance = new CEffect_MeshInst_Once(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_MeshInst_Once");
...
CCloneable* CEffect_MeshInst_Once::Clone(void* pArg)
{
	CEffect_MeshInst_Once* pInstance = new CEffect_MeshInst_Once(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_MeshInst_Once");
		Safe_Release(pInstance);
	}
...
void CEffect_MeshInst_Once::Free()
{
	super::Free();
