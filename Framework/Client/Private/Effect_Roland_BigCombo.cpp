...
CEffect_Roland_BigCombo::CEffect_Roland_BigCombo(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: super(pDevice, pContext)
{
}

CEffect_Roland_BigCombo::CEffect_Roland_BigCombo(const CEffect_Roland_BigCombo& Prototype)
	: super(Prototype)
{
}

HRESULT CEffect_Roland_BigCombo::Initialize_Prototype()
{
...
HRESULT CEffect_Roland_BigCombo::Initialize(void* pArg)
{
	super::DESC* pDesc = static_cast<super::DESC*>(pArg);
	if (nullptr == pDesc)
		return E_FAIL;

	pDesc->EffectDesc.GameObjectDesc.fRotationPerSec = -4.f;
...
void CEffect_Roland_BigCombo::Priority_Update(_float fTimeDelta)
{
	Compute_CamDistance();
}

void CEffect_Roland_BigCombo::Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
void CEffect_Roland_BigCombo::Late_Update(_float fTimeDelta)
{
	if (false == m_isActive)
		return;
...
HRESULT CEffect_Roland_BigCombo::Render()
{
...
HRESULT CEffect_Roland_BigCombo::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Roland_BigCombo::Render_Distortion()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
HRESULT CEffect_Roland_BigCombo::Ready_Components(const _wstring& strModelTag, const _wstring& strTextureTag, LEVELID eLevelID)
{
	/* Com_Shader */
...
CEffect_Roland_BigCombo* CEffect_Roland_BigCombo::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Roland_BigCombo* pInstance = new CEffect_Roland_BigCombo(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Roland_BigCombo");
...
CCloneable* CEffect_Roland_BigCombo::Clone(void* pArg)
{
	CEffect_Roland_BigCombo* pInstance = new CEffect_Roland_BigCombo(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Clone : CEffect_Roland_BigCombo");
		Safe_Release(pInstance);
	}
...
void CEffect_Roland_BigCombo::Free()
{
	super::Free();
