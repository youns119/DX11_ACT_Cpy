#include "pch.h"
#include "..\Public\Effect_Change_ExplToRiseW.h"

#include "GameInstance.h"

CEffect_Change_ExplToRiseW::CEffect_Change_ExplToRiseW(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CSpriteEffect{ pDevice, pContext }
{

}

CEffect_Change_ExplToRiseW::CEffect_Change_ExplToRiseW(const CEffect_Change_ExplToRiseW& Prototype)
	: CSpriteEffect{ Prototype }
{
}

HRESULT CEffect_Change_ExplToRiseW::Initialize_Prototype()
{
...
HRESULT CEffect_Change_ExplToRiseW::Initialize(void* pArg)
{
...
void CEffect_Change_ExplToRiseW::Priority_Update(_float fTimeDelta)
{
...
void CEffect_Change_ExplToRiseW::Update(_float fTimeDelta)
{
	m_fActiveDelayAcc -= fTimeDelta;
	if (m_fActiveDelayAcc > 0.f)
		return;
	...
	if ((m_fLifeTimeAcc >= m_fMaxLifeTime * 0.15f) && m_iPattern != CVIBuffer_InstancingParticle::RISE_WATER)
	{
		m_pVIBufferCom->Change_Pattern(CVIBuffer_InstancingParticle::RISE_WATER);
		m_iPattern = CVIBuffer_InstancingParticle::RISE_WATER;
	}
...
void CEffect_Change_ExplToRiseW::Late_Update(_float fTimeDelta)
{
	if (m_fActiveDelayAcc > 0.f)
		return;
...
HRESULT CEffect_Change_ExplToRiseW::Render()
{
...
HRESULT CEffect_Change_ExplToRiseW::Render_Revealage()
{
	if (m_pAdoptedMatrix == nullptr)
	{
		if (FAILED(m_pTransformCom->Bind_WorldMatrix_To_Shader(m_pShaderCom, "g_WorldMatrix")))
...
void CEffect_Change_ExplToRiseW::Clear_Info(_float fTimeDelta)
{
	...
	if (m_fLifeTimeAcc >= m_fMaxLifeTime)
	{
		...
		m_pVIBufferCom->Change_Pattern(CVIBuffer_InstancingParticle::EXPLOSION);
		m_iPattern = CVIBuffer_InstancingParticle::EXPLOSION;
	}
}

HRESULT CEffect_Change_ExplToRiseW::Ready_Components(const _wstring& strTextureTag, LEVELID eLevelID, CVIBuffer_Particle_Point::DESC& Desc)
{
	/* Com_Shader */
...
CEffect_Change_ExplToRiseW* CEffect_Change_ExplToRiseW::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CEffect_Change_ExplToRiseW* pInstance = new CEffect_Change_ExplToRiseW(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed To Created : CEffect_Change_ExplToRiseW");
...
CGameObject* CEffect_Change_ExplToRiseW::Clone(void* pArg)
{
	CEffect_Change_ExplToRiseW* pInstance = new CEffect_Change_ExplToRiseW(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX("Failed To Cloned : CEffect_Change_ExplToRiseW");
		Safe_Release(pInstance);
	}
...
void CEffect_Change_ExplToRiseW::Free()
{
	super::Free();
