...
void CMagnet::Update(_float _fTimeDelta)
{
    ...
    ImGui::Begin("PointAdd");
    if (ImGui::Checkbox("PointAdd", &m_bPointAdd)) {};
    if (ImGui::Button("PrevPointDelete"))
    {
        m_ControlPoint.pop_back();
    }
    if (ImGui::Button("Save_Point"))
    {
        _wstring szFile = L"../Bin/DataFiles/MagnetRail.data"; //다이얼로그로 가져오는 파일패스 저장할 곳
        ...
        for (auto& SavePoint : m_ControlPoint)
        {
            WriteFile(hFile, &SavePoint, sizeof(_float3), &dwByte, nullptr);
        }

        CloseHandle(hFile);
    }

    if (ImGui::Button("Load_Point"))
    {
        if (!m_ControlPoint.empty())
    ...
    if (m_bPointAdd) 
    {
        ...
        {
            _float3 vPos = {};
            m_pGameInstance->Compute_PixelPickPos(vPos);
            m_ControlPoint.push_back(vPos);
        }
    }
    ImGui::End();
...
HRESULT CMagnet::Ready_Components()
{
    ...
    /* Com_Collider */
    {
        ...
        Desc.vExtents = { 5.f, 20.f, 5.f };
        Desc.vCenter = { 0.f, -35.f, 0.f };
