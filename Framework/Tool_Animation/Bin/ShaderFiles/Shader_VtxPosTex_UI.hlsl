...
Texture2D g_CoverTexture;

struct UIParams
{
    float4 vSrcColor;       // 16바이트
    float4 vDstColor;       // 16바이트

    float2 vSpriteSize;     // 8바이트
    float2 vSpriteIndex;    // 8바이트

    float2 vStretchRange;   // 8바이트
    float fThreshold;       // 4바이트
    float fPadding;         // 4바이트
} g_UIParams;
...
VS_OUT_DISOLVE VS_MAIN_DISOLVE(VS_IN In)
{
    ...
    float2 spriteSize = 1.0f / g_UIParams.vSpriteSize;

    float2 texStart = g_UIParams.vSpriteIndex * spriteSize;
...
PS_OUT PS_MAIN_HURT(PS_IN In)
{
    ...
    Out.vColor.a *= g_UIParams.vSrcColor.a;
    
    // 색상 변형
    Out.vColor.rgb *= g_UIParams.vSrcColor.rgb;
...
PS_OUT PS_MAIN_STRETCH(PS_IN In)
{
    ...
    if (g_bStretchY)
    {
        ...
        if (In.vTexCoord.y < g_UIParams.vStretchRange.x / newTexLength)
            vStretchUV.y = In.vTexCoord.y * newTexLength;
        else if (In.vTexCoord.y >= (g_UIParams.vStretchRange.x / newTexLength) && In.vTexCoord.y <= 1 - (1 - g_UIParams.vStretchRange.y) / newTexLength)
            vStretchUV.y = g_UIParams.vStretchRange.x;
        else
            vStretchUV.y = (g_UIParams.vStretchRange.y + (In.vTexCoord.y - (1 - (1 - g_UIParams.vStretchRange.y) / newTexLength)) * newTexLength);
    }
    else
    {
        ...
        if (In.vTexCoord.x < g_UIParams.vStretchRange.x / newTexLength)
            vStretchUV.x = In.vTexCoord.x * newTexLength;
        else if (In.vTexCoord.x >= (g_UIParams.vStretchRange.x / newTexLength) && In.vTexCoord.x <= 1 - (1 - g_UIParams.vStretchRange.y) / newTexLength)
            vStretchUV.x = g_UIParams.vStretchRange.x;
        else
            vStretchUV.x = (g_UIParams.vStretchRange.y + (In.vTexCoord.x - (1 - (1 - g_UIParams.vStretchRange.y) / newTexLength)) * newTexLength);
    }
    Out.vColor = g_Texture.Sample(LinearSampler, vStretchUV);

    clip(Out.vColor.a - 0.2f);

    Out.vColor *= g_UIParams.vSrcColor;
...
PS_OUT PS_MAIN_STRETCH_BAR(PS_IN In)
{
    ...
    if (In.vTexCoord.x >= g_fCur / g_fMax && In.vTexCoord.x <= g_fLast / g_fMax)
        Out.vColor = g_UIParams.vDstColor;
...
PS_OUT PS_MAIN_STRETCH_BAR_EFFECT(PS_IN In)
{
    ...
    if (g_bEffectStart)
    {
        if (In.vTexCoord.x > g_fCur / g_fMax && In.vTexCoord.x < g_fPrev / g_fMax)
        {
            ...
            Out.vColor.rgb = lerp(float3(1, 1, 1), g_UIParams.vDstColor.rgb, 1 - fAlpha); // 흰색 → DstColor 보간
...
PS_OUT PS_MAIN_RING(PS_IN In)
{
    ...
    if (g_fRatio <= 0.f)
        Out.vColor = texColor * g_UIParams.vSrcColor;
    else
    {
        ...
        if (theta > g_fRatio)
        {
            Out.vColor.r = g_UIParams.vSrcColor.b * g_fRatio;
            Out.vColor.g = g_UIParams.vSrcColor.g;
            Out.vColor.b = g_UIParams.vSrcColor.b * (1 - g_fRatio);
...
PS_OUT PS_MAIN_FILTERBLEND(PS_IN In)
{
    ...
    float4 vFilterColor = g_UIParams.vSrcColor * (1.f - vTexelColor.a);
...
PS_OUT PS_MAIN_OPTIONBUTTON(PS_IN_DISOLVE In)
{
    ...
    if (g_bHovered)
    {
        ...
        clip(dissolveValue - g_UIParams.fThreshold);
        
        Out.vColor = vTexelColor * g_UIParams.vSrcColor;
    }
    else
    {
        clip(-1);
...
PS_OUT PS_MAIN_BODRER(PS_IN In)
{
    ...
    Out.vColor = float4(g_UIParams.vSrcColor.rgb, g_UIParams.vSrcColor.a * alpha);
...
PS_OUT PS_MAIN_COVER(PS_IN In)
{
    PS_OUT Out = (PS_OUT) 0;

    Out.vColor = g_Texture.Sample(LinearSampler, In.vTexCoord);

    clip(Out.vColor - 0.2f);

    float4 coverColor = g_CoverTexture.Sample(LinearSampler, In.vTexCoord);

    clip(coverColor.a - 0.2f);
    
    // 감마보정
    Out.vColor = pow(coverColor, 1.f / 2.2f);
...
PS_OUT PS_MAIN_SKILL(PS_IN In)
{
    ...
    {
        Out.vColor.rgb = g_UIParams.vSrcColor.rgb * Out.vColor.g;
    }
    else
    {
        Out.vColor = g_vBlackColor;
...
PS_OUT PS_MAIN_GAUGE(PS_IN In)
{
    ...
    if (vStretchUV.x < g_fCur / g_fMax)
        ...
        Out.vColor.rgb = g_UIParams.vDstColor.rgb;
    
    return Out;
}

PS_OUT PS_MAIN_SKILLEFFECT(PS_IN In)
{
    PS_OUT Out = (PS_OUT) 0;
...
PS_OUT PS_MAIN_COPYTEXTURE(PS_IN In)
{
    PS_OUT Out = (PS_OUT) 0;
    
    Out.vColor = g_Texture.Sample(LinearSampler, In.vTexCoord);
    clip(length(Out.vColor) - 0.0001);
    
    float dissolveValue = g_MaskTexture.Sample(LinearSampler, In.vTexCoord).r;
    Out.vColor.a = 1 - g_fThreshold;
...
//PS_OUT PS_MAIN_TEXTTEXTURE(PS_IN In)
//{
//    PS_OUT Out = (PS_OUT) 0;
//    
//    float wave = sin(In.vTexCoord.x * 10 + g_fTime * 2.0) * 0.02;
//    In.vTexCoord.y += wave;
//
//    
//    //// --- Scanline glitch
//    //float glitch = step(frac(In.vTexCoord.y * 1000 + g_fTime * 2), 0.5);
//    //In.vTexCoord.x += glitch * 0.005;
//    //
//    //// --- 알파 페이드용 간단한 타임 베이스 노이즈
//    //float fade = 0.5f + 0.5f * sin(dot(In.vTexCoord * 10, float2(12.989, 78.233)) + g_fTime * 2.0f);
//    //fade = saturate(fade);
//    
//    //Out.vColor = g_Texture.Sample(LinearSampler, In.vTexCoord);
//    //Out.vColor.a *= fade;
...
//    
//    // 시간 기반 흐름 오프셋
//    float2 flowUV = In.vTexCoord + float2(g_fTime * 0.05, g_fTime * 0.03) * 5.f; //g_fDistortionSpeed;
//
//    // 디스토션 텍스처에서 왜곡 벡터 추출
//    float2 distortion = g_MaskTexture.Sample(LinearSampler, flowUV).rg;
//    distortion = (distortion - 0.5f) * 2.0f;
//
//    // 최종 텍스처 좌표 계산
//    float2 finalUV = In.vTexCoord + distortion * 0.0175f;  //g_vDistortionScale;
//
//    // 텍스처 색상 샘플
//    Out.vColor = g_Texture.Sample(LinearSampler, finalUV);
//
//    return Out;
//}
...
//    PS_OUT Out = (PS_OUT) 0;
//
//    // 중심 기준 Y축 120만 사용 (720 기준)
//    float2 uv = In.vTexCoord;
//    uv.y = uv.y * 0.16666f + 0.41666f;
//
//    // 물결
//    float wave = sin(uv.x * 10 + g_fTime * 2.0) * 0.02;
//    uv.y += wave;
//
//    // 디스토션
//    float2 flowUV = uv + float2(g_fTime * 0.05, g_fTime * 0.03) * 5.f;
//    float2 distortion = g_MaskTexture.Sample(LinearSampler, flowUV).rg;
//    distortion = (distortion - 0.5f) * 2.0f;
//
//    float2 finalUV = uv + distortion * 0.0175f;
//
//    Out.vColor = g_Texture.Sample(LinearSampler, finalUV);
//    
//    Out.vColor = float4(frac(finalUV), 0, 1); // wrap 반복 확인용
//    return Out;
//}
PS_OUT PS_MAIN_TEXTTEXTURE(PS_IN In)
{
    ...
    // 중심 부분만 쓰기 (0.41666 ~ 0.58333)
    uv.y = lerp(0.41666f, 0.58333f, uv.y);
    ...
    // wave는 이 이후에 실제 샘플링 좌표에 직접 적용!
    finalUV.y += sin(finalUV.x * 10 + g_fTime * 2.0) * 0.02;
    
    //finalUV.y = (finalUV.y - 0.41666f) / (0.58333f - 0.41666f);
    
    // 샘플링
    Out.vColor = g_Texture.Sample(LinearSampler, finalUV);
    clip(Out.vColor.a - 0.1f);
    
    Out.vColor.a = g_fTextAlpha;
    //Out.vColor = float4(frac(finalUV), 0, 1); // wrap 반복 확인용
...
//28
    pass CoverPass
    {
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_MAIN_COVER();
    }
//29
    pass SkillEffectPass
    {
        SetRasterizerState(RS_Default);
        SetDepthStencilState(DSS_Skip_Z, 0);
        SetBlendState(BS_AlphaBlend, float4(0.f, 0.f, 0.f, 0.f), 0xffffffff);
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_MAIN_SKILLEFFECT();
    }
//30
    pass CopyTexturePass
    {
        SetRasterizerState(RS_Default);
        SetDepthStencilState(DSS_Skip_Z, 0);
        SetBlendState(BS_AlphaBlend, float4(0.f, 0.f, 0.f, 0.f), 0xffffffff);
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_MAIN_COPYTEXTURE();
    }
//31
    pass TextTexturePass
    {
        SetRasterizerState(RS_Default);
        SetDepthStencilState(DSS_Skip_Z, 0);
        SetBlendState(BS_AlphaBlend, float4(0.f, 0.f, 0.f, 0.f), 0xffffffff);
        ...
        VertexShader = compile vs_5_0 VS_MAIN();
        GeometryShader = NULL;
        PixelShader = compile ps_5_0 PS_MAIN_TEXTTEXTURE();
    }
}